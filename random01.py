...
def win_or_loss(multi, s, u):
    ...
    if s_temp == u:
        ...
        print('您输了！')
        user_info['glod'] -= (100 * multi)
    log_glod.append(user_info['glod'])
...
def buy_props(game_round):
    ...
    if user_info['glod'] % 400 == 0:
        ...
        if shop.lower() == 'yes':
            ...
            if good_num == '0':
                user_properties[str(game_round)] = 'X3'
                user_info['glod'] -= 100
                print('购买成功！消耗金币100')
            elif good_num == '1':
                user_properties[str(game_round)] = 'X5'
                user_info['glod'] -= 200
                print('购买成功！消耗金币200')
            else:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if (win_or_loss(multi, s, u) == 0):
            break
        buy_props(game_round)
    log_glod.sort(reverse=True)
    print('欢迎下次来玩，再见！最高金币：{}'.format(log_glod[0]))